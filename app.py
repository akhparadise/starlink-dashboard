...
def engineer_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["dl_deviation"]    = (
        (df[COL_DL] - df["dl_rolling_mean"]) /
        df["dl_rolling_mean"].replace(0, np.nan)
    )
...
def fuse_with_logs(df: pd.DataFrame, logs: pd.DataFrame) -> pd.DataFrame:
    ...
    rip_logs = logs[
        logs[desc_col].str.contains("Illegal unicast RIP|Illegal RIP", case=False, na=False)
    ].copy()
    if rip_logs.empty:
        return df

    rip_logs   = rip_logs.dropna(subset=["_ts"])
    df_sorted  = df.sort_values(COL_TS).reset_index(drop=True)
    rip_sorted = rip_logs.sort_values("_ts")

    merged = pd.merge_asof(
        df_sorted,
        rip_sorted[["_ts", desc_col]].rename(
            columns={"_ts": "_log_ts", desc_col: "_log_event"}
        ),
...
# ─────────────────────────────────────────────────────────────
FEATURE_COLS = [
    COL_PING, COL_DROP, COL_SNR, COL_DL,
...
def run_isolation_forest(df: pd.DataFrame, contamination: float = 0.06):
    df = df.copy()
...
def classify_incidents(row, z_threshold: float = 2.5) -> str:
    if row["if_score"] != -1:
        return "STABLE"
    if row[COL_DROP] > 0.05 and row.get("nms_illegal_rip", False):
        return "CYBER ATTACK"
    if row["correlation_score"] > 0.65 and row["z_drop"] > z_threshold:
        return "CYBER ATTACK"
    if row[COL_SNR] < 0.5 or row["z_snr"] > z_threshold:
        return "SIGNAL JAMMING"
    if row["z_ping"] > z_threshold:
        return "NETWORK CONGESTION"
    return "NETWORK CONGESTION"

def full_analysis(df, logs, contamination=0.06, z_threshold=2.5):
    df = engineer_features(df)
    df = fuse_with_logs(df, logs)
    df, model, scaler = run_isolation_forest(df, contamination)
    df["status"] = df.apply(lambda r: classify_incidents(r, z_threshold), axis=1)
    return df, model, scaler

# ─────────────────────────────────────────────────────────────
# ATTACK SIMULATION
# ─────────────────────────────────────────────────────────────
def simulate_advanced_attack(df: pd.DataFrame, logs: pd.DataFrame):
    df_sim     = df.copy()
    n          = len(df_sim)
    attack_idx = list(range(max(0, n - 12), n))
    rng        = np.random.default_rng(seed=42)

    df_sim.loc[df_sim.index[attack_idx], COL_DROP] = rng.uniform(0.12, 0.20, len(attack_idx))
    df_sim.loc[df_sim.index[attack_idx], COL_PING] = (
        df_sim[COL_PING].median() * rng.uniform(2.1, 3.0, len(attack_idx))
    )
    df_sim.loc[df_sim.index[attack_idx], COL_DL] = (
        df_sim[COL_DL].median() * rng.uniform(0.05, 0.15, len(attack_idx))
    )

    attack_times = df_sim[COL_TS].iloc[attack_idx].values
    syn_logs = pd.DataFrame({
        "_ts":               pd.to_datetime(attack_times),
        "Event Description": ["Illegal unicast RIP message from 10.0.0.1"] * len(attack_times),
        "Severity":          ["CRITICAL"] * len(attack_times),
        "Element Name":      ["SIM-INJECT"] * len(attack_times),
    })
    merged_logs = (
        pd.concat([logs, syn_logs], ignore_index=True)
        if not logs.empty else syn_logs
    )
...
def explain_incident(row: pd.Series, scaler: StandardScaler) -> dict:
    ...
    raw        = np.abs(scaled)
    normed     = raw / (raw.sum() + 1e-9)
    return {FEATURE_LABELS[f]: float(normed[i]) for i, f in enumerate(FEATURE_COLS)}

# ─────────────────────────────────────────────────────────────
# PLOTLY HELPERS
# ─────────────────────────────────────────────────────────────
def ts_plot_with_anomalies(df, y_col, y_label, title) -> go.Figure:
    fig = go.Figure()
    ...
    for t in df[df["status"] == "CYBER ATTACK"][COL_TS]:
        fig.add_vrect(
...
def xai_bar_chart(contributions: dict, incident_status: str) -> go.Figure:
    ...
    fig = go.Figure(go.Bar(
        ...
        marker=dict(
            ...
            opacity=[min(1.0, 0.4 + 0.6 * v) for v in vals]
        ),
        text=[f"{v*100:.1f}%" for v in vals],
        textposition="outside",
        textfont=dict(color="#CDD9E5", size=10)
    ))
    # ── два update_layout — устраняет конфликт ключа xaxis из PLOTLY_TEMPLATE ──
    fig.update_layout(**PLOTLY_TEMPLATE)
    fig.update_layout(
        title=dict(text="Feature Attribution (XAI)",
...
# ─────────────────────────────────────────────────────────────
with st.sidebar:
    st.markdown('<div class="section-label">Data Sources</div>',
                unsafe_allow_html=True)
    csv_files = st.file_uploader(
        "Telemetry CSV (30-day export)",
        type="csv", accept_multiple_files=True
    )
    xlsx_file = st.file_uploader(
        "NMS Security Log (.xlsx)", type=["xlsx", "xls"]
    )

    st.markdown('<div class="section-label" style="margin-top:20px">Simulation Controls</div>',
                unsafe_allow_html=True)
    ...
    st.markdown('<div class="section-label" style="margin-top:20px">Detection Parameters</div>',
                unsafe_allow_html=True)
...
if not csv_files:
    ...
    ⬡ &nbsp; Upload one or two 30-day telemetry CSV files and optionally
    the NMS security log (.xlsx) to begin analysis.
    </div>
    """, unsafe_allow_html=True)
    schema = pd.DataFrame({
        "Column":      [COL_TS, COL_DL, COL_UL, COL_PING,
                        COL_DROP, COL_SNR, COL_OBSTR],
        "Type":        ["datetime","float","float","float",
                        "float (0-1)","float","float (0-1)"],
        "Description": [
            "UTC timestamp", "Downlink throughput (Mbps)",
            "Uplink throughput (Mbps)", "Avg POP ping latency (ms)",
            "Avg packet drop rate", "Avg receive SNR", "Obstruction % time"
        ]
    })
...
for i, f in enumerate(csv_files[:2]):
    all_dfs.append(load_telemetry(f))
    all_names.append(region_labels[i] if i < 2 else f"REGION {i+1}")

if st.session_state.get("simulation_active", False):
    st.markdown("""
    <div class="alert-critical">
    ⚡ STRESS TEST MODE ACTIVE — Synthetic DoS injected into final 12 data points
...
for name, df_raw in zip(all_names, all_dfs):
    df_out, _, scaler_out = full_analysis(df_raw, logs_df, contamination, z_threshold)
    df_out["REGION_TAG"] = name
...
c2.metric("CYBER ATTACKS",     total_attack,
          delta="HIGH RISK" if total_attack > 5 else "LOW RISK",
          delta_color="inverse")
c3.metric("SIGNAL JAMMING",    total_jam)
c4.metric("CONGESTION EVENTS", total_cong)
c5.metric("THREAT INDEX",      f"{threat_pct}%",
          delta="↑ ELEVATED" if threat_pct > 5 else "↓ NORMAL",
          delta_color="inverse")
...
tab_global, tab_forensic, tab_raw = st.tabs([
    "GLOBAL ANALYTICS", "FORENSIC ANALYSIS", "RAW TELEMETRY"
])
...
with tab_global:
    for region_name, df_region in analyzed.items():
        st.markdown(f'<div class="section-label">{region_name}</div>',
                    unsafe_allow_html=True)
        col_chart, col_pie = st.columns([3, 1])

        with col_chart:
            st.plotly_chart(
                ts_plot_with_anomalies(
        ...
        with col_pie:
            st.plotly_chart(status_pie(df_region), use_container_width=True)
            badge_map = {
                "STABLE": "badge-stable", "CYBER ATTACK": "badge-attack",
                "SIGNAL JAMMING": "badge-jamming",
                "NETWORK CONGESTION": "badge-congestion"
            }
            for s, cnt in df_region["status"].value_counts().items():
                cls = badge_map.get(s, "badge-stable")
                st.markdown(f'<span class="{cls}">{s}: {cnt}</span><br>',
                             unsafe_allow_html=True)

        col_drop, col_thr = st.columns(2)
        with col_drop:
            st.plotly_chart(
                ts_plot_with_anomalies(df_region, COL_DROP,
                                       "Drop Rate", "Packet Drop Rate"),
                use_container_width=True
            )
        with col_thr:
            st.plotly_chart(
                ts_plot_with_anomalies(df_region, COL_DL,
                                       "Throughput (Mbps)", "Downlink Throughput"),
                use_container_width=True
            )
    ...
    if len(analyzed) == 2:
        st.markdown(
            ...
            'Multi-Region Comparative View</div>',
            unsafe_allow_html=True
        )
        fig_comp = px.line(
            combined_df, x=COL_TS, y="correlation_score",
            color="REGION_TAG",
        ...
        fig_comp.add_hline(
            y=0.65, line_dash="dot", line_color="#F85149",
            annotation_text="Attack threshold (0.65)",
            annotation_font_color="#F85149"
        )
        fig_comp.update_layout(**PLOTLY_TEMPLATE)
...
# ═══════════════════════════════════════════════
with tab_forensic:
    st.markdown('<div class="section-label">Incident Selection</div>',
                unsafe_allow_html=True)

    col_sel1, col_sel2 = st.columns([1, 2])
    with col_sel1:
        region_sel = st.selectbox("Region", list(analyzed.keys()),
                                  key="forensic_region")
    ...
    if anomaly_df.empty:
        ...
        with col_sel2:
            incident_idx = st.selectbox(
                ...
                format_func=lambda i: (
                    f"{df_sel.loc[i, COL_TS].strftime('%Y-%m-%d %H:%M')}  ·  "
                    f"{df_sel.loc[i, 'status']}"
                )
            )
        ...
        s      = row["status"]
        rip    = bool(row.get("nms_illegal_rip", False))

        badge_map = {
            "CYBER ATTACK": "badge-attack",
            "SIGNAL JAMMING": "badge-jamming",
            "NETWORK CONGESTION": "badge-congestion"
        }
        ...
        <div style="background:#0D1117;border:1px solid #21262D;
          ...
          <span style="font-family:'JetBrains Mono',monospace;color:#8B949E;
                       font-size:0.8rem;margin-left:12px">
            {row[COL_TS].strftime('%Y-%m-%d %H:%M UTC')}
          </span>
          <br><br>
          <span style="color:#CDD9E5;font-size:0.85rem">
            IF score: <b style="color:#F85149">{row['if_raw']:.4f}</b>
            &nbsp;|&nbsp; Drop: <b>{row[COL_DROP]*100:.2f}%</b>
            &nbsp;|&nbsp; Latency: <b>{row[COL_PING]:.1f} ms</b>
            &nbsp;|&nbsp; SNR: <b>{row[COL_SNR]:.3f}</b>
            &nbsp;|&nbsp; NMS RIP:
            <b style="color:{'#F85149' if rip else '#3FB950'}">
              {'YES' if rip else 'NO'}
            </b>
          </span>
        </div>
        ...
        with col_xai:
            st.plotly_chart(xai_bar_chart(contributions, s),
                            use_container_width=True)
        ...
        with col_rules:
            ...
            rules_map = {
                "CYBER ATTACK": [
                    ...
                    f"✓ NMS Illegal RIP: {'YES' if rip else 'NO'}",
                    f"✓ Correlation Score: {row['correlation_score']:.3f} (thr 0.65)",
                ],
                "SIGNAL JAMMING": [
                    "✓ IsolationForest: anomaly flag",
                    f"✓ SNR: {row[COL_SNR]:.3f} (thr < 0.5)",
                    f"✓ Z-score SNR: {row['z_snr']:.2f} (thr {z_threshold})",
                ],
                "NETWORK CONGESTION": [
                    "✓ IsolationForest: anomaly flag",
                    f"✓ Ping Z-score: {row['z_ping']:.2f} (thr {z_threshold})",
                    "✓ No NMS log correlation",
                ]
            }
            for rule in rules_map.get(s, []):
                st.markdown(
                    f"<span style='font-size:0.82rem;color:#CDD9E5'>{rule}</span>",
                    unsafe_allow_html=True
                )
        ...
        st.markdown(
            '<div class="section-label" style="margin-top:16px">'
            'Incident Context Window (±2h)</div>',
            unsafe_allow_html=True
        )
        t0     = row[COL_TS]
        window = df_sel[
        ...
        fig_ctx.add_trace(
            go.Scatter(x=window[COL_TS], y=window[COL_DROP],
                       mode="lines+markers", name="Drop Rate",
                       line=dict(color="#58A6FF", width=1.5)),
            row=1, col=1
        )
        fig_ctx.add_trace(
            go.Scatter(x=window[COL_TS], y=window[COL_PING],
                       mode="lines+markers", name="Latency",
                       line=dict(color="#E3B341", width=1.5)),
            row=2, col=1
        )

        # ── add_shape вместо add_vline (обход бага Plotly + pandas Timestamp) ──
        t0_str = t0.isoformat()
        ...
        fig_ctx.update_layout(**PLOTLY_TEMPLATE)
        fig_ctx.update_layout(height=340, showlegend=False)
        st.plotly_chart(fig_ctx, use_container_width=True)

        # Z-score summary table
        st.markdown(
            '<div class="section-label" style="margin-top:8px">'
            'Statistical Profile (Context Window)</div>',
            unsafe_allow_html=True
        )
        z_summary = pd.DataFrame({
            "Metric": [
                "Ping Latency Z-score", "Packet Drop Z-score",
                "SNR Z-score", "Correlation Score"
            ],
            "Incident Value": [
            ...
            "Window Mean": [
                f"{window['z_ping'].mean():.3f}",
                f"{window['z_drop'].mean():.3f}",
                f"{window['z_snr'].mean():.3f}",
                f"{window['correlation_score'].mean():.3f}"
            ],
            "Threshold": [
                f">{z_threshold}", f">{z_threshold}",
                f">{z_threshold}", ">0.65"
            ],
            "Triggered": [
...
# ═══════════════════════════════════════════════
with tab_raw:
    st.markdown('<div class="section-label">Full Processed Dataset</div>',
                unsafe_allow_html=True)

    region_filter = st.selectbox(
        "Region", ["ALL"] + list(analyzed.keys()), key="raw_region"
    )
    ...
    display_cols = [
        COL_TS, COL_PING, COL_DROP, COL_DL, COL_SNR, COL_OBSTR,
        "z_ping", "z_drop", "correlation_score",
        "if_raw", "nms_illegal_rip", "status"
    ]
    ...
    st.caption(
        f"Showing {len(view_df):,} rows · "
        f"{(view_df['status'] != 'STABLE').sum()} anomalies"
    )
